...
public class SpawnManager : MonoBehaviour
{
    ...
    public List<GameObject> PaintPrefab;
    public RoomManager RoomManager;
    
    Transform northWest, northEst, southWest;
    
    int paintRoom;
    public int[] windowRoom;
    int paintIndex;
        ...
        for (int j = 0; j < windowRoom.Length; j++)
        {
            for (int i = 0; i < WindowRooms.Length; i++)
            {
                if (WindowRooms[i].RoomIndex == windowRoom[j])
                {
                    ...
                    WindowRooms[i].Window[windowIndex[j]].gameObject.GetComponent<TriggerClimb>().roomManager = RoomManager;
                }
            }
        }
    ...
    private void SpawnPaint(int room)
    {
        for (int j = 0; j < 2; j++)
        {
            prefabIndex = Random.Range(0, PaintPrefab.Count);
            ...
            PaintPrefab.Remove(PaintPrefab[prefabIndex]);
        }
    }
}
